package com.ctp.view.components.drawingboard.paint {
	...
	public class BrushBoard extends Sprite {
		...
		public function get isEmpty(): Boolean {
			var bmd: BitmapData = new BitmapData(numBorderW, numBorderH, false, 0xFFFFFF);
		...
		private function stageMouseDownHandler(e:MouseEvent):void {
			//if (brush.type == BrushType.PAINT) {
				//if (toolBarMovie.bgMovie.hitTestPoint(mouseX, mouseY, true)) {
					//return;
				//}
				//var bmd: BitmapData = new BitmapData(numBorderW, numBorderH, true, 0x00000000);
				//bmd.draw(graphicMovie);
			...
			this.addEventListener(MouseEvent.ROLL_OUT, stageMouseUpHandler);
			...
			if (brush.type == BrushType.BRUSH) {
				...
				if (brush.type == BrushType.AIR_BRUSH) {
					cachedGraphic = new BitmapData(numBorderW, numBorderH, true, 0);
		...
		private function stageMouseUpHandler(e:Event):void {
			...
			this.removeEventListener(MouseEvent.ROLL_OUT, stageMouseUpHandler);
		...
		private function draw(): void {
			var bmd: BitmapData = new BitmapData(numBorderW, numBorderH, true, 0);
			bmd.draw(graphicMovie);
			graphicMovie.graphics.clear();
			if (brush.type == BrushType.ERASER) {
				bmd.draw(hiddenMovie, null, null, BlendMode.ERASE);
			} else if (brush.type == BrushType.AIR_BRUSH) {
				var hidden: BitmapData = new BitmapData(numBorderW, numBorderH, true, 0);
