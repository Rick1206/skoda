...
package com.pyco.controls {
	...
	public class ScrollBar extends Sprite {
		...
		private var _wheelSpeed: Number = 2;
		...
		private function reInitVertical(needToReset: Boolean = true): void {
			if(contentMovie && needToReset) contentMovie.y = contentStart;
			enableVScrollbar();
			setValues(needToReset);
		}
		
		/** reInit values in horizontal mode */
		private function reInitHorizontal(): void {
			if(contentMovie) contentMovie.y = contentStart;
			enableHScrollbar();
			setValues();
		}
		...
		private function arrowPressHandler(e: MouseEvent): void {
			//trace("Scrollbar arrowPressHandler upMovie: " + upMovie);
		...
		private function arrowPressUpdateHandler(event: Event): void {
			...
			//trace("ScrollBar.arrowPressUpdateHandler 222: " + isSnapping + " / " +  distanceSnapping + " / " + contentRange + " / " + percent);
		...
		private function mouseWheelHandler(e: MouseEvent): void {
			if (useMouseWheel && this.maskMovie.hitTestPoint(stage.mouseX, stage.mouseY, false) && this.visible && this.parent.visible) {
		...
		private function setValues(needToReset: Boolean = true): void {
			if (mode == MODE_VERTICAL) setVerticalValues();
			else setHorizontalValues();
			if (needToReset) percent = 0;
		}
		
		private function setVerticalValues(needToReset: Boolean = true): void {
			if (contentMovie) {
				...
				if (needToReset) contentMovie.y = contentStart;	
		...
		private function setHorizontalValues (): void {
			if (contentMovie) {
				...
				if (sliderResizable) {
					if (rotateFromVerticalMode) {
						sliderMovie.height = Math.ceil((maskMovie.width / temp) * trackMovie.height);
						sliderMovie.height = Math.max(trackMovie.height / 20, sliderMovie.height);
					} else {
		...
		//use for textfield
		public function updateScrollBar(needToReset: Boolean = true): void {
			...
			if (contentText) {
				...
				if (!_isInit) {
					throw Error ("reInit is called before update");
					return;
				}
				var contentY: int = contentMovie.y;
				stopSliding(null);
				if (mode == MODE_VERTICAL) {
					reInitVertical(needToReset);
					if (this.visible) {
						var percentValue: Number = Math.abs(contentY - contentStart) / contentRange;
						percentage = Math.min( 1, Math.max( 0, percentValue ) );
						...
						contentMovie.y = Math.max(contentY, contentStart - contentRange);
					}
				} else {
					reInitHorizontal();
				}
		...
		private function updateContentText():void {
			//trace("updateContentText : " + contentText + " / " + willPreventUpdateScroll + " / " + percent);
